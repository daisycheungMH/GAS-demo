...
import { Group, AvailabilityBlock, AvailabilityStatus } from "../types";
...
import { availabilityBlockstoStatusCode, SheetAvailability } from "../lib/data_interfaces";
import { pushAvailabilities } from "../lib/sheets";
...
export default function AvailabilityTab({ group, currentUser, onSyncNeeded }: AvailabilityTabProps) {
  ...
  const handleCellPaint = (day: string, hour: string, brush: AvailabilityStatus = activeBrush) => {
    ...
    saveBlocksToSheets(updatedBlocks); // NOTDONE, replace with google
  };
  ...
  const saveBlocksToSheets = async (newAvailability: AvailabilityBlock[]) => { // NOTDONE, get rid of this
      setDraftAvailability(newAvailability);
    setSaveStatus("saving");
    try {
      const newNewBlocks: number[] = []
      for (const block of newAvailability) {
        const blockStart = parseInt(block.start.split(":")[0]);
        const blockEnd = parseInt(block.end.split(":")[0]);
        if (blockEnd - blockStart <= 0) {
          console.warn(`Invalid block with start ${block.start} and end ${block.end}`);
          continue;
        }
        for (var i = blockStart; i < blockEnd; i++) {
          newNewBlocks[i] = block.status === "available" ? 2 : block.status === "maybe" ? 1 : 0;
        }
      }
      const newStatusCode = availabilityBlockstoStatusCode(newNewBlocks);
      const newSheetAvailability: SheetAvailability = {
        UUID: "", // This will be generated by the backend
        member: currentUser,
        date: selectedMode === "recurring" ? "recurring" : getSpecificDateForDay(0, selectedWeekOffset),
        statusCode: newStatusCode
      }
      pushAvailabilities([newSheetAvailability]);
